...
import { editUser } from '../../resources';
...
const EditUser = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      try {
        const formData = new FormData(); // crea un objeto con clave y valor(ej:{"name","Juan"})
  ...
  return !token ? (
    <Navigate to="/" /> // esto no esta haciendo nada....
  ) : (
    <>
      <section className="formwithbuttons">
        <form id="editUserForm" className="pageform" onSubmit={handleSubmit}>
          <img
            id="imagen"
            src={
            ...
            <>
              {loggedUser.avatar === 1 && (
                <>
                  ...
                  <label htmlFor="deleteavatar" className="deleteavatar">
                    Eliminar avatar
                  </label>
                </>
              )}
          ...
          <input
            ...
            required
          ></input>
          <button className="bigbutton">Guardar cambios</button>
        </form>
