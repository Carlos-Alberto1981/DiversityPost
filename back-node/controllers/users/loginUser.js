...
const loginUser = async (req, res, next) => {
  try {
    ...
    res.cookie('token', token); //se utiliza para establecer el valor del nombre de la cookie.
    res.setHeader('token', token); // se utiliza para enviar el token en la cabecera en vez de el body
    res.send({
      status: 'ok',
      data: {
        id: user.id,
